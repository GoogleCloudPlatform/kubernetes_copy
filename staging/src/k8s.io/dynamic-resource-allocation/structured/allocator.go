...
type constraint interface {
	...
	add(requestName, subRequestName string, device *draapi.CompositeDevice, deviceID DeviceID) bool

	// For every successful add there is exactly one matching removed call
	// with the exact same parameters.
	remove(requestName, subRequestName string, device *draapi.CompositeDevice, deviceID DeviceID)
}
...
func (m *matchAttributeConstraint) add(requestName, subRequestName string, device *draapi.CompositeDevice, deviceID DeviceID) bool {
...
func (m *matchAttributeConstraint) remove(requestName, subRequestName string, device *draapi.CompositeDevice, deviceID DeviceID) {
	if m.requestNames.Len() > 0 && !m.matches(requestName, subRequestName) {
		// Device not affected by constraint.
		return
	}
