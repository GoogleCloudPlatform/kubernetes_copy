...
func TestValidateClaim(t *testing.T) {
	...
	scenarios := map[string]struct {
		...
		"prioritized-list-invalid-nested-request": {
			...
			claim: func() *resource.ResourceClaim {
				claim := testClaim(goodName, goodNS, validClaimSpecWithFirstAvailableOf)
				claim.Spec.Devices.Requests[0].FirstAvailableOf[0] = resource.DeviceSubRequest{
					Name: badName,
				}
				return claim
			}(),
		...
		"prioritized-list-nested-requests-same-name": {
			...
			claim: func() *resource.ResourceClaim {
				claim := testClaim(goodName, goodNS, validClaimSpecWithFirstAvailableOf)
				claim.Spec.Devices.Requests[0].FirstAvailableOf[1].Name = goodName
				return claim
			}(),
		},
		"prioritized-list-too-many-subrequests": {
			...
			claim: func() *resource.ResourceClaim {
				...
				claim.Spec.Devices.Requests[0].FirstAvailableOf = subRequests
				return claim
			}(),
		},
